...
Format.prototype.immediateRefresh = function()
{
	...
	if (graph.isSelectionEmpty())
	{
		...
		if (window.drawflag) // 这个在地图中不添加绘图标签
		{
		...
		else
		{
			...
			diagramPanel = div.cloneNode(false);
...
ArrangePanel.prototype.addGeometry = function(container)
{
	...
	{
		widthUpdate.apply(this, arguments);
	}, this.getUnitStep(), null, null, true);
	var height = this.addUnitInput(div, this.getUnit(), 16, 52, function()
	{
		heightUpdate.apply(this, arguments);
	}, this.getUnitStep(), null, null, true);
	...
	widthUpdate = this.addGeometryHandler(width, function(geo, value, cell)
	{
		...
		else if (geo.width > 0)
		{
			...
			var value = Math.max(0, panel.fromUnit(value));
...
TextFormatPanel.prototype.addFont = function(container)
{
	...
	var inputUpdate = this.installInputHandler(input, mxConstants.STYLE_FONTSIZE, Menus.prototype.defaultFontSize, 0.1, 999, ' pt',
	function(fontSize)
	{
	...
	var stepper = this.createStepper(input, inputUpdate, .1, 10, true, Menus.prototype.defaultFontSize, true);
...
DiagramFormatPanel.prototype.addGridOption = function(container)
{
    ...
    var stepper = this.createStepper(input, update, 1, null, null, null, true);
	...
	function update(evt)
	{
		...
		value = fPanel.fromUnit(Math.max(fPanel.inUnit(0.1), (isNaN(value)) ? fPanel.inUnit(10) : value));
		// if (value <= 0) {
        //     value = 0.1;
        // }
	...
	stepper.style.marginTop = (mxClient.IS_MAC && mxClient.IS_GC) ? '-16px' : ((mxClient.IS_WIN) ? '-18px' : '-17px');
