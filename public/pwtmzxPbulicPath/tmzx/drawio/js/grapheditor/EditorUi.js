 ...
 */
EditorUi = function(editor, container, lightbox, svgParser)
{
...
EditorUi.prototype.initCanvas = function()
{
	...
	graph.getPreferredPageSize = function(bounds, width, height)
	{
		if (window.drawflag) {
			...
			return new mxRectangle(0, 0, w, h);
		} else {
		...
		if (!graph.isViewer())
		{
			...
			graph.sizeDidChange = function()
			{
				if (window.drawflag)
				{
					...
					graph.minimumGraphSize = new mxRectangle(0, 0, w, h);
...
EditorUi.prototype.refresh = function(sizeDidChange)
{
	...
	{
		// let svgParser = this.svgParser;
		// let map_param = svgParser.map_param;
		// let {lt, sw: cur_sw, sh:cur_sh} = svgParser.currentExtent();
		this.refreshOverlay();
	}
	else
	{
...
EditorUi.prototype.refreshOverlay = function() {
	let svgParser = this.svgParser;
	if (svgParser && svgParser.graph) {
		let map_param = svgParser.map_param;
		let {lt, sw: cur_sw, sh:cur_sh, width, height} = svgParser.currentExtent();
		let w = cur_sw;
		let h = cur_sh

		this.diagramContainer.style.left =  `${lt.x}px`;
		this.diagramContainer.style.top = `${lt.y}px`;
		this.diagramContainer.style.width = `${w}px`;
		this.diagramContainer.style.height = `${h}px`;
		this.diagramContainer.style.right = 'auto';
		this.diagramContainer.style.bottom = 'auto';
		this.diagramContainer.style.overflow = 'hidden';


		let scale = cur_sw / width;
		// svgParser.graph.view.setTranslate(0, 0);
		svgParser.graph.view.setScale(scale);
	}
}
/**
...
EditorUi.prototype.createDivs = function()
{
	...
	this.hsplit.style.position = 'absolute';
	this.hsplit.style.cursor = 'col-resize';
...
{
	if (this.svgParser && this.svgParser.stencilDoc)
	{
		return new Sidebar(this, container, this.svgParser.stencilDoc);
	}
	else
	{
		return new Sidebar(this, container, null);
	}
};
