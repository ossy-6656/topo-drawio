...
function lineInterpolate(line) {
  return function (t) {
    return t === 0
      ...
      : t === 1
        ? line[1]
        : pointTranslate(line[0], lineAngle(line), lineLength(line) * t)
  }
}
...
function polygonHull(points) {
  ...
  for (var i0 = 0; i0 < pointsCopy.length; i0++) {
    while (
      lower.length >= 2 &&
      cross(lower[lower.length - 2], lower[lower.length - 1], pointsCopy[i0]) <= 0
    ) {
      lower.pop()
    }
  ...
  for (var i1 = pointsCopy.length - 1; i1 >= 0; i1--) {
    while (
      upper.length >= 2 &&
      cross(upper[upper.length - 2], upper[upper.length - 1], pointsCopy[i1]) <= 0
    ) {
      upper.pop()
    }
...
geometric.lineAngle = lineAngle
geometric.lineInterpolate = lineInterpolate
geometric.lineLength = lineLength
geometric.lineMidpoint = lineMidpoint
geometric.pointRotate = pointRotate
geometric.pointTranslate = pointTranslate
geometric.polygonArea = polygonArea
geometric.polygonBounds = polygonBounds
geometric.polygonCentroid = polygonCentroid
geometric.polygonHull = polygonHull
geometric.polygonLength = polygonLength
geometric.polygonMean = polygonMean
geometric.polygonRegular = polygonRegular
geometric.polygonRotate = polygonRotate
geometric.polygonScale = polygonScale
geometric.polygonScaleX = polygonScaleX
geometric.polygonScaleY = polygonScaleY
geometric.polygonTranslate = polygonTranslate
geometric.lineIntersectsLine = lineIntersectsLine
geometric.lineIntersectsPolygon = lineIntersectsPolygon
geometric.pointInPolygon = pointInPolygon
geometric.pointOnPolygon = pointOnPolygon
geometric.pointLeftofLine = pointLeftofLine
geometric.pointRightofLine = pointRightofLine
geometric.pointOnLine = pointOnLine
geometric.pointWithLine = pointWithLine
geometric.polygonInPolygon = polygonInPolygon
geometric.polygonIntersectsPolygon = polygonIntersectsPolygon
geometric.angleReflect = angleReflect
geometric.angleToDegrees = angleToDegrees
geometric.angleToRadians = angleToRadians
