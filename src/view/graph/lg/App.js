/**
 * App.js - 力光正交图编辑器主应用类
 * 基于 drawio 的 mxGraph 框架，用于编辑电力系统的正交图
 *
 * 主要功能：
 * 1. 管理 SVG 图形解析和渲染
 * 2. 提供图元拖拽、编辑、删除等功能
...
import SvgGenerate from "./SvgGenerate.js";
// import {saveGraph} from "@/api/tmzx/svg/index.ts";
// import { checkEditZjtPermission } from '@/api/tmzx/abnormalchange/index.ts'

// 导入自定义图形处理工具
import GraphHandler from "@/plugins/tmzx/graph/GraphHandler.js";      // 图形处理器
import GraphTool from "@/plugins/tmzx/graph/GraphTool.js";           // 图形工具类
import geometric from "@/plugins/tmzx/geometric.js";                 // 几何计算工具
import mathutil from "@/plugins/tmzx/mathutil.js";                   // 数学工具
import StationHandler from '@/plugins/tmzx/graph/StationHandler.js'  // 站点处理器
import TextUtil from '@/plugins/tmzx/graph/TextUtil.js'              // 文本工具
import DeviceCategoryUtil from '@/plugins/tmzx/graph/DeviceCategoryUtil.js' // 设备分类工具
import $bus from '@/utils/bus';                                        // 事件总线
import { ElMessage } from 'element-plus'                               // 消息提示

/**
 * App 构造函数 - 初始化正交图编辑器
 * @param {Object} editor - mxGraph 编辑器实例
 * @param {HTMLElement} container - 容器元素，用于渲染图形编辑器
 * @param {Boolean} lightbox - 是否启用灯箱模式（弹窗显示）
 * @param {Object} svgParser - SVG 解析器实例
 *
 * 功能说明：
 * 1. 继承自 EditorUi，获得基本的图形编辑功能
 * 2. 设置网格颜色、页面可见性等配置
 * 3. 初始化各种事件监听器
 */
let App = function (editor, container, lightbox, svgParser)
{
    // 设置应用标识，用于区分不同类型的应用
    this.appFlag = '正交图';

    // 调用父类 EditorUi 的构造函数，初始化基本的编辑器 UI
    EditorUi.call(this, editor, container, (lightbox != null) ? lightbox :
        (urlParams['lightbox'] == '1' || (uiTheme == 'min' &&
            urlParams['chrome'] != '0')), svgParser);

    // 将 SVG 解析器的 UI 引用指向当前 App 实例
    svgParser.ui = this;
    ...
    // ==================== 窗口卸载事件监听 ====================
    // 当用户关闭窗口时，如果文件有未保存的修改，记录日志（用于 Google Drive 集成）
    ...
    let scrollEvt = mxUtils.bind(this,function () {
        this.scrollLeft = graph.container.scrollLeft;
        this.scrollTop = graph.container.scrollTop;
    });
    graph.panningHandler.addListener(mxEvent.PAN_END, scrollEvt)
    view.addListener(mxEvent.SCALE,  scrollEvt);
    mxEvent.addListener(graph.container, 'scroll',  scrollEvt);
    this.load();
};
 ...
 */
App.main = function (callback, createUi, container, svgParser)
{
    try
    {
        ...
        {
            EditorUi.logError('Graph Global Monitor - Global: ' + ((message != null) ? message : ''),
                url, linenumber, colno, err, null, true);

            if (window.console != null && !EditorUi.isElectronApp)
            {
                console.error('Graph Global Monitor - Message:', message, '\nURL:', url, '\nLine:',
                    linenumber, '\nColumn:', colno, '\nError:', err);
            }
            else
            {
                mxLog.show();
                mxLog.debug('Graph Global Monitor - Message:', message, '\nURL:', url, '\nLine:',
                    linenumber, '\nColumn:', colno, '\nError:', err);
            }
            ...
            {
                alert('Graph Global Monitor - Error: ' + ((message != null) ? message : ''));
            }, 100);
        };
        ...
        // Removes info text in embed mode，这个地方也用不着
        ...
        // Redirects to the latest AWS icons，这个地方也用不着
        ...
        if (window.mxscript != null)
        {
            ...
            //     CryptoJS != null && AppGraph.mode != AppGraph.MODE_DROPBOX && AppGraph.mode != AppGraph.MODE_TRELLO)
            // {
            ...
            if (urlParams['plugins'] != '0' && urlParams['offline'] != '1')
            {
                ...
                if (temp != null)
                {
                    // Mapping from key to URL in AppGraph.plugins
                    ...
                    function realMain()
                    {
                        try
                        {
                            ...
                            if (Editor.currentTheme != '' && Editor.currentTheme != 'kennedy' &&
                                Editor.currentTheme != 'dark' && mxUtils.indexOf(Editor.themes, Editor.currentTheme) < 0)
                            {
                            ...
                            {
                                EditorUi.debug('AppGraph.start', ['v' + EditorUi.VERSION, ui, (new Date().getTime() - t0.getTime()) + 'ms']);
    ...
    {
        let graphCon = document.querySelector('#graphCon');
        graphCon.innerHTML = '';
        var pre = document.createElement('pre');
        mxUtils.write(pre, e.stack);
        graphCon.appendChild(pre);
    }
};
...
if (urlParams['embed'] != '1')
{
    ...
    App.prototype.menubarHeight = 31;
}
else
{
...
// 自定义操作，包括间隔平均分布、站房美化
App.prototype.addCustomAction = function () {
    let graph = this.editor.graph;
    ...
    // 站房美化通用方法
    function beautifyStationCommon()
    {
        graph.escape();

        let cell = graph.getSelectionCell();
        let stationCell = GraphTool.getStationGroup(graph, cell);
        ...
        let busList = GraphTool.getBus(graph, stationCell); // 站房下面可能有多个母线
        if (busList.length == 0)
        {
            return;
        }

        // let stationState = view.getState(stationCell);
        // model.beginUpdate();
        // try {
        //
        // } finally {
        ...
        for (let busCell of busList) {
            let edges = model.getEdges(busCell);
            for (let edge of edges) {
                GraphHandler.beautifyGapCommon(graph, busCell, edge);
            }
        }
    }
    // 站房优化
    var action1 = this.actions.addAction('prettifyStation', mxUtils.bind(this, function(evt, trigger)
        {
            ...
            {
                // prettifyStation();
                beautifyStationCommon();
            }
        })
    );

    // 间隔平均分布通用方法
    function gapEvenDistributionHandler() {
        graph.escape();

        let symbolProp = svgParser.symbolProp;
        let cell = graph.getSelectionCell();
        let stationCell = GraphTool.getStationGroup(graph, cell);
        ...
        {
            // 计算母线两端坐标
            // 根据正交原则，及母线线特性来计算母线两端坐标
            for(let busCell of busList)
            {
                // 计算母线两端坐标
                let p0 = GraphTool.getTouchPoint(graph, busCell, 0, .5);
                let p1 = GraphTool.getTouchPoint(graph, busCell, 1, .5);

                let edges = model.getEdges(busCell);
                let line = [[p0.x, p0.y], [p1.x, p1.y]];
                let interpolate = geometric.lineInterpolate(line);

                // 把间隔按顺序排列，按与起点距离比较
                edges.sort((item1, item2) => {

                    let _p1 = GraphTool.getBusTouchPointCommon(graph, item1); // 母线与连接线连接点
                    let _p2 = GraphTool.getBusTouchPointCommon(graph, item2); // 母线与连接线连接点

                    let len1 = mathutil.pixelLen(p0, _p1);
                    let len2 = mathutil.pixelLen(p0, _p2);

                    return len1 < len2 ? -1 : 1;
                });

                let tmp = 0.1 / 2;
                let step = (1 - 0.1) / (edges.length - 1);
                for(let edge of edges)
                {
                    // 检查当前连接线是否连接两个母线
                    let flag = GraphTool.isMultiBusConnection(graph, edge, busCell);
                    let p = interpolate(tmp);
                    let curVec = new Vector2(p[0], p[1]);
                    GraphHandler.GapEvenDistributionCommon(graph, symbolProp, tmp, p0, p1, curVec, busCell, edge, flag);
                    tmp += step;
                }
            }
        }
    ...
    // 站房内间隔平均分布
    var action2 = this.actions.addAction('pjffjg', mxUtils.bind(this, function(evt, trigger) {
            var evt = (trigger != null) ? trigger : evt;
    ...
    // 优化站外出线
    var action9 = this.actions.addAction('prettifyOutterLine', mxUtils.bind(this, function(evt, trigger)
        {
            // Context menu click uses trigger, toolbar menu click uses evt
            var evt = (trigger != null) ? trigger : evt;
            ...
            {
                let cell = graph.getSelectionCell();
                let groupCell = GraphTool.getStationGroup(graph, cell);
                ...
                if (busList == 0) {
                    ElMessageBox.alert(`站房缺失母线，无法定位`, '提示', {
                ...
                try {
                    ...
                    StationHandler.prettifyOutgoingLine(graph, groupCell, busList, gap10);
                } finally {
                    model.endUpdate();
                }
            }
    ...
    // 垂直化文本
    var action3 = this.actions.addAction('verticalText', mxUtils.bind(this, function(evt, trigger)
        {
            var evt = (trigger != null) ? trigger : evt;
            ...
            else
            {
                ...
                try
                {
                    ...
                    if (cell.style.indexOf('text;') != -1) {

                        let list = TextUtil.getVerticalTextArr(cell.value);
                        let dims = TextUtil.getTextDimensionFromTxtList(fs, list);
                        model.setValue(cell, list.join('\n'));

                        let geometry = graph.getCellGeometry(cell);
                        if (geometry) {

                            geometry.width = dims.width;
                            geometry.height = dims.height + fs * 2;
    ...
    // 水平化文本
    var action4 = this.actions.addAction('horizontalText', mxUtils.bind(this, function(evt, trigger)
        {
            var evt = (trigger != null) ? trigger : evt;
    ...
    // 选择母线及关联设备
    var action5 = this.actions.addAction('selectBusGroup', mxUtils.bind(this, function(evt, trigger)
        {
            var evt = (trigger != null) ? trigger : evt;
            ...
            else
            {
                ...
                try
                {
                    ...
                    let list = GraphTool.getBusGroup(graph, cell);
    ...
    // 选择站内同类设备
    var action6 = this.actions.addAction('selectSimilarDevice', mxUtils.bind(this, function(evt, trigger)
        {
            var evt = (trigger != null) ? trigger : evt;
            ...
            else
            {
                ...
                {
                    let cell = graph.getSelectionCell();
                    let list = StationHandler.selectSimilarCell(graph, cell);
                    graph.setSelectionCells(list);
                }
                finally
                {
    ...
    // 选择站内同类设备文本
    var action7 = this.actions.addAction('selectSimilarDeviceTxt', mxUtils.bind(this, function(evt, trigger)
        {
            var evt = (trigger != null) ? trigger : evt;
            ...
            else
            {
                ...
                {
                    let cell = graph.getSelectionCell();
                    let list = StationHandler.selectSimilarCellText(graph, cell);
                    graph.setSelectionCells(list);
                }
                finally
                {
    ...
    // 选择站内同类设备文本
    var action8 = this.actions.addAction('selectSimilarDevicePoint', mxUtils.bind(this, function(evt, trigger)
        {
            var evt = (trigger != null) ? trigger : evt;
            ...
            else
            {
                ...
                {
                    let cell = graph.getSelectionCell();
                    let list = StationHandler.selectSimilarCellPoint(graph, cell);
                    graph.setSelectionCells(list);
                }
                finally
                {
    ...
    // 正交快捷键
    this.actions.addAction('orthoLine', mxUtils.bind(this, function ()
    {
        // this.menus.edgeStyleChange(menu,
        //     '', [mxConstants.STYLE_EDGE, mxConstants.STYLE_CURVED, mxConstants.STYLE_NOEDGESTYLE],
        //     [null, null, null],
        //     'geIcon geSprite geSprite-straight', null, true)
        //     .setAttribute('title', mxResources.get('straight'));

        let keys = [mxConstants.STYLE_EDGE, mxConstants.STYLE_CURVED, mxConstants.STYLE_NOEDGESTYLE];
        ...
        try
        {
            ...
            if (cells.length > 0) {
                ...
                if (cell.style) {
                    ...
                    if (styleObj['edgeStyle'] == 'orthogonalEdgeStyle') {
    ...
    // 选择站内所有文本
    var action9 = this.actions.addAction('selectAllTxt', mxUtils.bind(this, function(evt, trigger) {
            var evt = (trigger != null) ? trigger : evt;
            ...
            else
            {
                ...
                try
                {
                    ...
                    let list = StationHandler.getAllText(graph, stationCell);
                    graph.setSelectionCells(list);
                }
                finally
                {
    ...
    // 所有测点
    var action10 = this.actions.addAction('selectAllPoint', mxUtils.bind(this, function(evt, trigger) {
            var evt = (trigger != null) ? trigger : evt;
            ...
            else
            {
                ...
                try
                {
                    ...
                    let list = StationHandler.getAllText(graph, stationCell, 'point');
                    graph.setSelectionCells(list);
                }
                finally
                {
    ...
    // 所有名称标签
    var action11 = this.actions.addAction('selectAllName', mxUtils.bind(this, function(evt, trigger) {
            var evt = (trigger != null) ? trigger : evt;
            ...
            else
            {
                ...
                try
                {
                    ...
                    let list = StationHandler.getAllText(graph, stationCell, 'name');
                    graph.setSelectionCells(list);
                }
                finally
                {
    ...
    // 杆塔优化
    var action12 = this.actions.addAction('beautifyPole', mxUtils.bind(this, function(evt, trigger)
        {
            var evt = (trigger != null) ? trigger : evt;
            ...
            else
            {
                ...
                try
                {
                    ...
                    let symbolProp = svgParser.symbolProp;
                    let gap2 = width; // 设置2米间距
                    let gapTxt = width / 4; // 设置1米间距
                    // 优化柱上设备
                    GraphHandler.reCalculateOnePoleMountedTransformer(graph, cell, symbolProp, gap2, gapTxt);
                }
                finally
                {
                    model.endUpdate();
    ...
    // 缩放多个设备
    this.actions.addAction('scaleMulti', mxUtils.bind(this, function(evt, trigger)
        {
            let cellList = graph.getSelectionCells();
            ...
            for(let cell of cellList) {
                if (model.isVertex(cell)) {
                    ...
                    let isGroup = GraphTool.isGroupCell(graph, pCell);
                    if (isGroup) {
                        continue;
                    }

                    if (scaleMap.has(cell))
                    {
                        continue;
                    }
                    ...
                    isGroup = GraphTool.isGroupCell(graph, cell);
                    let obj = {
                        cx: geo.getCenterX(),
                        cy: geo.getCenterY(),
                        isGroup: isGroup,
                    ...
                    scaleMap.set(cell, obj);
                    scaleList.push(cell);

                    let cellId;
                    if (isGroup) {
                        let stationCell = GraphTool.getStationCell(graph, cell);
                        cellId = stationCell.id;
                    } else {
                        cellId = cell.id;
                    }

                    let txtId = 'TXT-' + cellId;
                    let txtCell = model.getCell(txtId);
                    if (txtCell) {
                        let styleObj = TextUtil.parseDrawioStyle(txtCell.style);
                        let fs = +styleObj.fontSize;
                        let txtGeo = model.getGeometry(txtCell);
                        let txtObj = {
                            cx: txtGeo.getCenterX(),
                            cy: txtGeo.getCenterY(),
                            width: txtGeo.width,
                            height: txtGeo.height,
                            fs
                        }
                        scaleMap.set(txtCell, txtObj);
                    }

                    let pointId = 'Point-' + cellId;
                    let pointCell = model.getCell(pointId);
                    if (pointCell) {
                        let styleObj = TextUtil.parseDrawioStyle(pointCell.style);
                        let fs = +styleObj.fontSize;
                        let pointGeo = model.getGeometry(pointCell);
                        let txtObj = {
                            cx: pointGeo.getCenterX(),
                            cy: pointGeo.getCenterY(),
                            width: pointGeo.width,
                            height: pointGeo.height,
                            fs
                        }
                        scaleMap.set(pointCell, txtObj);
                    }

                }
            }

            $bus.emit('multiScale_zjt', {
                scaleMap, scaleList
            });
        })
    );
...
App.prototype.createKeyHandler = function(editor)
{
    var editorUi = this;
    ...
    keyHandler.isEventIgnored = function(evt)
    {
        ...
        let ignored = !(mxEvent.isShiftDown(evt) && evt.keyCode == 9) &&
                        ...
                        if (handler != null)
                        {
                            ...
                            thread = window.setTimeout(function()
                            {
                                if (handler.first != null)
                                {
                                    ...
                                    let list = handler.cells;

                                    for(let c of list) {
                                        if (model.isVertex(c) && !c.isText) {
                                            let sbid = c.id;
                                            let poiId = 'Point-' + sbid;
                                            if (!txtSet.has(poiId)) {
                                                cellMoveHandler(poiId, dx, dy);
                                            }
                                            ...
                                            if (c.style) {
                                                if (c.style.indexOf('group;') != -1) {
                                                    let stationCell = GraphTool.getStationCell(graph, c);
                                                    if (stationCell) {
                                                        let stationTxtId = 'TXT-' + stationCell.id;
                                                        if (!txtSet.has(stationTxtId)) {
                                                            cellMoveHandler(stationTxtId, dx, dy);
                                                        }
                                                    }
                                                }
                                            }
    ...
    mxKeyHandler.prototype.getFunction = function(evt)
    {
        if (graph.isEnabled())
        {
            ...
            else if (mxEvent.isAltDown(evt) && evt.keyCode == 81) {
                action = editorUi.actions.get('orthoLine');
            }
            ...
            else if (directions[evt.keyCode] != null && !graph.isSelectionEmpty())
            {
                ...
                else
                {
                    ...
                    if (this.isControlDown(evt))
                    {
                        return function()
                        {
                            // 禁用鼠标和ctrl或者shiftn改变设备大小
                            // nudge(evt.keyCode, (mxEvent.isShiftDown(evt)) ? graph.gridSize : 10, true);
                        };
                    }
                    else
                    {
                        return function()
                        {
                            nudge(evt.keyCode, (mxEvent.isShiftDown(evt)) ? graph.gridSize : 1);
                        };
                    }
                }
            }
    ...
    keyHandler.bindAction(81, true, 'orthoLine'); // Ctrl+Q  正交线，自个加入的
...
App.prototype.getCurrentScrollPosition = function () {
    let editor = this.editor;
    let graph = editor.graph;
    return {
        sl: graph.container.scrollLeft,
        st: graph.container.scrollTop
    }
}
    ...
    try
    {
        ...
        window.onerror = function (message, url, linenumber, colno, err)
        {
            ...
            if (message != 'ResizeObserver loop limit exceeded' && message != 'ResizeObserver loop completed with undelivered notifications.')
            {
                ...
                // console.log('App.js ->', 'Uncaught: ' + ((message != null) ? message : ''), url, linenumber, colno, err);
                // ui.handleError({message: message}, mxResources.get('unknownError'), null, null, null, null, true);
                console.log({message: message}, mxResources.get('unknownError'), null, null, null, null, true)
            }
        };
...
App.prototype.filterDrafts = function (filePath, guid, callback)
{
    ...
    try
    {
        ...
        {
            EditorUi.debug('AppGraph.filterDrafts', [this], 'items', items);
 ...
 */
App.prototype.saveFile_bak = function (forceDialog, success)
{
    var file = this.getCurrentFile();
    var prev = this.mode;
...
// 设置svg静态文本
App.prototype.setSvgTxtObj = function (svgTxtObj) {
...
App.prototype.saveFile = function (forceDialog, success)
{
    ...
    //     } else {
            ...
            // saveGraph(param).then(res => {
            //     if(!res.msg){
            //         ElMessage.success('正交图保存成功！')
            //     } else{
            //         ElMessage.error('正交图保存失败:' + res.msg)
            //     }
            // })
    //     }
    // })
...
App.prototype.fileCreated = function (file, libs, replace, done, clibs, success)
{
    ...
    if (this.spinner.spin(document.querySelector('#graphCon'), mxResources.get('inserting')))
    {
...
App.prototype.updateHeader = function ()
{
    if (this.menubar != null)
    {
        ...
        var logo = 'url(./drawio/img/grid.png)';
        ...
        this.appIcon.style.height = '32px';
        ...
        var updateBackground = mxUtils.bind(this, function ()
        {
            ...
            this.appIcon.style.backgroundColor = '#000';
        });
         ...
         */
        var right = (Editor.currentTheme != 'atlas' && urlParams['embed'] != '1') ? 0 : 10;
        ...
        if (urlParams['embed'] != '1' && Editor.currentTheme != 'atlas')
        {
            ...
            this.toggleElement.style.right = '0px';
...
App.prototype.getCanvasData = function(canvas, xml, format, ignorePageName, dpi)
{
    var data = this.createImageDataUri(canvas, xml, format, dpi);
    return data;
};

App.prototype.getImageData = function(callback)
{
    let scale = 1;
    ...
    let dpi = null;
    let theme = 'light';
    let exportType = 'diagram';

    let graph = this.editor.graph;

    let list = graph.getVerticesAndEdges(true, false);
    let bounds = graph.getBoundingBoxFromGeometry(list, true);

    let scale1 = 400 / bounds.width;
    let scale2 = 200 / bounds.height;
    ...
    })))
    {
        ...
        try
        {
            this.editor.exportToCanvas(mxUtils.bind(this, function(canvas) {
                    ...
                    try
                    {
                        let data = this.getCanvasData(canvas, (editable) ? this.getFileData(true, null,
