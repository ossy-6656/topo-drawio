...
export default class LGSvgParser extends SvgBase {
    ...
    newparsePolygon(node) {
        const graph = this.graph;
        const attrMap = this.attrMap;
        ...
        const ObjectID = $node.attr('id') || `poly_${Date.now()}`;
        const propsDom = $node.find('polygon').parent().find('metadata');
        const propMap = this.getPropMap(propsDom);
        metaMap.set(ObjectID, propsDom.html() || '');
    
        const lineNode = $node.find('polygon').first();
        const pointsRaw = lineNode.attr('points') || '';
        const pointArr = pointsRaw.trim().split(/\s+/).filter(pt => pt);
        if (!pointArr.length) return null;
    
        // 1. 全部坐标乘缩放系数
        const points = pointArr.map(ptStr => {
            const [xStr = '0', yStr = '0'] = ptStr.split(',');
            return {
                x: (parseFloat(xStr) || 0) * scale,
                y: (parseFloat(yStr) || 0)
            };
        });
        // 2. 生成缩放后的points字符串，放入style（核心修复，避免双重偏移）
        const scaledPoints = points.map(p => `${p.x},${p.y}`).join(' ');
    
        // 3. 计算包围盒（用于vertex容器x/y/宽高）
        ...
        const lmp = { x: minX, y: minY };
    
        // 样式属性容错
        const cls = lineNode.attr('class') || '';
        let stroke = lineNode.attr('stroke') || '#000';
        let strokeWidth = lineNode.attr('stroke-width');
        strokeWidth = /^\d+(\.\d+)?$/.test(strokeWidth) ? strokeWidth : '1';
    
        propMap.cls = cls;
        propMap.strokeWidth = strokeWidth;
        propMap.stroke = stroke;
        propMap.points = pointsRaw; // 保留原始字符串存属性映射
    
        // 全局样式覆盖
        const curStyle = styleObj[cls];
        if (curStyle && curStyle.stroke) {
            stroke = curStyle.stroke;
        }
    
        const styleSb = [];
        styleSb.push(`id=${ObjectID};`);
        const pathStr = `M${scaledPoints} Z`;
        styleSb.push('shape=path;');
        styleSb.push(`path=${pathStr};`);
        // styleSb.push('shape=polygon;'); // 使用mx原生多边形
        styleSb.push('flag=station;whiteSpace=wrap;');
        styleSb.push('rotation=0;rotatable=0;');
        
        // 关键：写入缩放完成的坐标，不再使用原始pointsRaw
        // styleSb.push(`points=${scaledPoints};`);
        styleSb.push(`strokeColor=${stroke};`);
        styleSb.push(`strokeWidth=${strokeWidth};`);
        styleSb.push('fillColor=none;');
        // 锁定不可编辑移动
        styleSb.push('expand=0;editable=0;movable=0;resizable=0;deletable=0;locked=1;connectable=0;');
        ...
        // 删除 cell.points = points; mxGraph完全不读取，冗余无效代码
        attrMap.set(cell.id, propMap);
        return cell;
    }

    createSvgPolygon(node) {
      var graph = this.graph;
      let attrMap = this.attrMap;
      let parent = graph.getDefaultParent();
      let metaMap = this.metaMap;
      ...
      Object.keys(style).forEach(key => poly.style[key] = style[key]);
      return poly;
    }

    /**
     * 解析 facGToSvg 生成的 <circle> 形状（ACLineSegment_Layer 中的连接点/节点等）
     */
    parseCircleShape(devGroupNode) {
        const graph = this.graph;
        const attrMap = this.attrMap;
        const parent = graph.getDefaultParent();
        const metaMap = this.metaMap;
        const scale = this.getScale();
        const $g = $(devGroupNode);
        const $circle = $g.find('circle').first();
        if (!$circle.length) return null;

        const gId = $g.attr('id') || '';
        const propsDom = $g.find('metadata');
        const propMap = this.getPropMap(propsDom);
        let ObjectID, ObjectName, PSRType;
        if (propMap && propMap['cge:PSR_Ref']) {
            ObjectID = propMap['cge:PSR_Ref'].ObjectID || gId;
            ObjectName = propMap['cge:PSR_Ref'].ObjectName || '';
            PSRType = propMap['cge:PSR_Ref'].PSRType || '36000000';
        } else {
            ObjectID = gId;
            ObjectName = '';
            PSRType = '36000000';
        ...
        const cx = (parseFloat($circle.attr('cx')) || 0) * scale;
        const cy = (parseFloat($circle.attr('cy')) || 0) * scale;
        const r = (parseFloat($circle.attr('r')) || 10) * scale;
        const x = cx - r;
        const y = cy - r;
        const w = r * 2;
        const h = r * 2;

        let stroke = $circle.attr('stroke') || '#000';
        let strokeWidth = $circle.attr('stroke-width') || '1';
        let fill = $circle.attr('fill') || 'none';
        if (fill === 'transparent') fill = 'none';

        const styleSb = [];
        styleSb.push(`id=${ObjectID};`);
        styleSb.push('shape=ellipse;');
        styleSb.push('flag=ellipse;whiteSpace=wrap;');
        styleSb.push(`strokeColor=${stroke};`);
        styleSb.push(`strokeWidth=${strokeWidth};`);
        styleSb.push(`fillColor=${fill};`);
        styleSb.push('rotation=0;rotatable=0;');
        styleSb.push('connectable=0;');

        const cell = graph.insertVertex(parent, ObjectID, null, x, y, w, h, styleSb.join(''));
        cell.setConnectable(false);
        cell.psrtype = PSRType;
        if (ObjectName) cell.name = ObjectName;
        attrMap.set(cell.id, propMap || {});
        return cell;
    }

    parseRectShape(devGroupNode) {
        const graph = this.graph;
        const attrMap = this.attrMap;
        const parent = graph.getDefaultParent();
        const metaMap = this.metaMap;
        ...
        const x = (parseFloat($rect.attr('x')) || 0) * scale;
        const y = (parseFloat($rect.attr('y')) || 0) * scale;
        const w = (parseFloat($rect.attr('width')) || 10) * scale;
        const h = (parseFloat($rect.attr('height')) || 10) * scale;

        let stroke = $rect.attr('stroke') || '#000';
        let strokeWidth = $rect.attr('stroke-width') || '1';
        let fill = $rect.attr('fill') || 'none';
        if (fill === 'transparent') fill = 'none';

        const styleSb = [];
        styleSb.push(`id=${ObjectID};`);
        styleSb.push('shape=rect;');
        styleSb.push('flag=rect;whiteSpace=wrap;');
        styleSb.push(`strokeColor=${stroke};`);
        styleSb.push(`strokeWidth=${strokeWidth};`);
        styleSb.push(`fillColor=${fill};`);
        styleSb.push('rotation=0;rotatable=0;');
    ...
    /**
     * 解析 facGToSvg 生成的 <polygon> 形状（ACLineSegment_Layer 中的菱形等）
     */
    parsePolygonShape(devGroupNode) {
        const graph = this.graph;
        const attrMap = this.attrMap;
        const parent = graph.getDefaultParent();
        const metaMap = this.metaMap;
        const scale = this.getScale();
        const $g = $(devGroupNode);
        const $polygon = $g.find('polygon').first();
        ...
        const pointsRaw = $polygon.attr('points') || '';
        const pointArr = pointsRaw.trim().split(/\s+/).filter(pt => pt);
        if (!pointArr.length) return null;
        ...
        let stroke = $polygon.attr('stroke') || '#000';
        let strokeWidth = $polygon.attr('stroke-width') || '1';
        let fill = $polygon.attr('fill') || 'none';
        if (fill === 'transparent') fill = 'none';

        const styleSb = [];
        styleSb.push(`id=${ObjectID};`);
        styleSb.push('shape=rhombus;');
        styleSb.push('flag=polygon;whiteSpace=wrap;');
        styleSb.push(`strokeColor=${stroke};`);
        styleSb.push(`strokeWidth=${strokeWidth};`);
        styleSb.push(`fillColor=${fill};`);
        styleSb.push('rotation=0;rotatable=0;');
        ...
        const cell = graph.insertVertex(parent, ObjectID, null, minX, minY, width, height, styleSb.join(''));
        cell.setConnectable(false);
        cell.psrtype = PSRType;
        if (ObjectName) cell.name = ObjectName;
        attrMap.set(cell.id, propMap || {});
    ...
    getMinFontSize(glist) {
        ...
        /**
     * 解析 facGToSvg 生成的 poke 热点（Hot_Layer 中的跳转链接）
     */
    parsePoke(node) {
        const graph = this.graph;
        const attrMap = this.attrMap;
        ...
        const $g = $(node);
        const $rect = $g.find('rect').first();
        const $text = $g.find('text').first();
        if (!$rect.length) return null;

        const gId = $g.attr('id') || '';

		// 从 data-* 属性读取 poke 跳转数据（避免命名空间选择器兼容性问题）
		const ahref = node.getAttribute('data-ahref') || '';
		const keyNameFromMeta = node.getAttribute('data-keyname') || '';

		const label = $text.length ? ($text.text() || '').trim() : (keyNameFromMeta || gId);

        // 读取 metadata 中的 PSRef
        const propsDom = $g.find('metadata');
        const propMap = this.getPropMap(propsDom);
        let ObjectID = gId;
        let ObjectName = label;
        let PSRType = '0399';
        if (propMap && propMap['cge:PSR_Ref']) {
            ObjectID = propMap['cge:PSR_Ref'].ObjectID || gId;
            ObjectName = propMap['cge:PSR_Ref'].ObjectName || label;
            PSRType = propMap['cge:PSR_Ref'].PSRType || '0399';
        }
        metaMap.set(ObjectID, propsDom.html() || '');

        const x = (parseFloat($rect.attr('x')) || 0) * scale;
        const y = (parseFloat($rect.attr('y')) || 0) * scale;
        const w = (parseFloat($rect.attr('width')) || 100) * scale;
        const h = (parseFloat($rect.attr('height')) || 50) * scale;
        ...
        // 文本样式
        const fs = parseFloat($text.attr('font-size')) || 14;
        const textFill = $text.attr('fill') || '#fff';

        const styleSb = [];
        styleSb.push(`id=${ObjectID};`);
        styleSb.push('shape=rect;');
        styleSb.push('flag=poke;whiteSpace=wrap;');
        styleSb.push('layer=Hot_Layer;');
        styleSb.push(`strokeColor=${stroke};`);
        styleSb.push(`strokeWidth=${strokeWidth};`);
        styleSb.push(`fillColor=${fill};`);
        styleSb.push(`fontColor=${textFill};`);
        styleSb.push(`fontSize=${fs * scale};`);
        styleSb.push('rotation=0;rotatable=0;');
        styleSb.push('connectable=0;');

        const cell = graph.insertVertex(parent, ObjectID, label, x, y, w, h, styleSb.join(''));
        cell.setConnectable(false);
        cell.psrtype = PSRType;
        cell.name = ObjectName;
		if (ahref) {
			cell.superlinkname = keyNameFromMeta || label;
			cell.superlinkpsrid = '';
			cell.href = ahref;
		}
        attrMap.set(cell.id, propMap || {});
                    ...
                    if (layerId === 'substation_layer') {
                        ...
                        if (node) {
                            ...
                            if (nodeName === 'polyline') {
                                ...
                                cell = this.parseRectShape(devGroupNode);
                                if (cell) {
                                    devList.push(cell);
                                }
                            } else if (nodeName === 'circle') {
                                cell = this.parseCircleShape(devGroupNode);
                                if (cell) {
                                    devList.push(cell);
                                }
                            } else if (nodeName === 'polygon') {
                                cell = this.parsePolygonShape(devGroupNode);
                                if (cell) {
                                    devList.push(cell);
                                }
                            }
