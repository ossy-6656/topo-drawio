...
/**
 * 图元SVG内容包装分组+电力模型元数据
 * @param {Element} dom 原始自定义图元dom节点
 * @param {string} useInner 转换后的svg图形内部片段
 * @returns {string} 带元数据的svg <g> 分组字符串
 */
function wrapDeviceUseGroup(dom, useInner) {
    // 1. 提取基础标识
    const rawId = dom.getAttribute('id');
    const devId = rawId || 'PD_unknown';
    // 设备名称优先级：keyname > keyid > id
    const rawKeyName = dom.getAttribute('keyname') || dom.getAttribute('keyid') || devId;
    // 统一转义
    const safeId = escapeXmlAttr(devId);
    const safeName = escapeXmlAttr(rawKeyName);
    // 获取设备PSR类型
    const psrType = psrTypeForGNode(dom.nodeName);
    // 标准化内部图形片段，兜底空字符串
    const innerSvg = normalizeFacUseFragment(useInner) || '';

    // 2. 动态适配图层（区分设备类型，不再硬编码Breaker）
    let layerName = 'Common_Layer';
    const tagName = dom.nodeName.toLowerCase();
    if (tagName === 'diamond') {
        layerName = 'Breaker_Layer';
    } else if (tagName === 'energyconsumer') {
        layerName = 'Load_Layer';
    }
    const safeLayer = escapeXmlAttr(layerName);
    ...
    const fragArr = [
        ...
        innerSvg,
        '<metadata>',
        `<cge:PSR_Ref ObjectID="${safeId}" ObjectName="${safeName}" PSRType="${psrType}"/>`,
        `<cge:Layer_Ref ObjectName="${safeLayer}"/>`,
        '</metadata>',
        '</g>'
    ];
    return fragArr.join('');
...
/**
 * 原始单图形svg标签转为带g、metadata、Shape_Layer标准格式
 * @param {string} rawSvg 原始<polygon>/<rect>等单图形字符串
 * @returns {string} 转换完成的完整<g>结构字符串
 */
function convertShapeToCgeFormat(rawSvg) {
    // 1. 创建dom解析器
    const parser = new DOMParser();
    const xmlDoc = parser.parseFromString(rawSvg, "image/svg+xml");
    const shapeEl = xmlDoc.documentElement;
    // 获取图元ID
    const id = shapeEl.getAttribute("id");
    if (!id) throw new Error("图形标签缺少id属性");

    // 处理transform，统一补充 scale(1,1)
    let transform = shapeEl.getAttribute("transform") || "";
    if (!transform.includes("scale")) {
        transform = transform ? `${transform} scale(1,1)` : "scale(1,1)";
    }
    shapeEl.setAttribute("transform", transform);

    // 固定窗口编号wm=17
    shapeEl.setAttribute("wm", "17");

    // 转图形内部html字符串
    const shapeHtml = shapeEl.outerHTML;

    // 组装标准外层g + metadata，图层Shape_Layer
    const result = `
        <g id="${id}">
            ${shapeHtml}
            <metadata>
                <cge:PSR_Ref ObjectID="${id}" />
                <cge:Layer_Ref ObjectName="Shape_Layer" />
            </metadata>
        </g>
    `.trim();
...
function wrapPokeGroup(dom) {
    const id = dom.getAttribute('id') || 'POKE_unknown';
    const oid = escapeXmlAttr(id);
    const x = parseFloat(dom.getAttribute('x')) || 0;
    const y = parseFloat(dom.getAttribute('y')) || 0;
    const w = parseFloat(dom.getAttribute('w')) || 100;
    const h = parseFloat(dom.getAttribute('h')) || 50;
    const ahref = dom.getAttribute('ahref') || '';
    const keyname = dom.getAttribute('key_name') || dom.getAttribute('keyid') || '';
    const lc = dom.getAttribute('lc');
    const fc = dom.getAttribute('fc');
    const fm = dom.getAttribute('fm');
    const fs = dom.getAttribute('fs') || dom.getAttribute('p_FontHeight') || '14';

    let rect = '<rect x="' + x + '" y="' + y + '" width="' + w + '" height="' + h + '" ';
    rect += 'fill="' + (fm !== '0' && fc ? 'rgb(' + fc + ')' : 'none') + '" ';
    rect += 'stroke="none" ';
    rect += '/>';

    // 文本居中于矩形
    const textX = x + w / 2;
    const textY = y + h / 2 + parseFloat(fs) / 3;
    let text = '<text x="' + textX + '" y="' + textY + '" ';
    text += 'fill="' + (lc ? 'rgb(' + lc + ')' : '#fff') + '" ';
    text += 'font-size="' + fs + '" ';
    text += 'text-anchor="middle" ';
    text += 'flag="text" ';
    text += '>';
    text += escapeXmlAttr(keyname);
    text += '</text>';

    // 用 data-* 属性传递 poke 跳转信息，避免命名空间元素兼容性问题
    return '<g id="' + oid + '" flag="poke" data-ahref="' + escapeXmlAttr(ahref) + '" data-keyname="' + escapeXmlAttr(keyname) + '">' + rect + text + '<metadata>' + '<cge:PSR_Ref ObjectID="' + oid + '"/>' + '<cge:Layer_Ref ObjectName="Hot_Layer"/>' + '</metadata></g>';
}
...
function buildLgCompatibleBody(children, onWarn) {
    ...
    for (let i = 0; i < children.length; i++) {
        ...
        switch (nodeName) {
            ...
            case 'diamond':
            {
                const inner = SymbolParse.parseDiamond(dom);
                if (inner) lineParts.push(wrapLineLikeGroup(dom, inner));
                break;
            }
            case 'circle': {
                const inner = SymbolParse.parseCircle(dom);
                if (inner) lineParts.push(wrapLineLikeGroup(dom, inner, i));
                break;
            }
            case 'rect': {
                const inner = SymbolParse.parseRect(dom);
                if (inner) lineParts.push(wrapLineLikeGroup(dom, inner, i));
                break;
            }
            case 'poke': {
                const inner = wrapPokeGroup(dom);
                if (inner) pokeParts.push(inner);
                break;
            }
            default:
                if (onWarn) onWarn(nodeName, dom.getAttribute('id'));
    ...
    return {
        body:  `<g id="Hot_Layer">${pokeParts.join('')}</g>` + `<g id="Breaker_Layer">${deviceParts.join('')}</g>`+ `<g id="ACLineSegment_Layer">${lineParts.join('')}</g>` + `<g id="Text_Layer">${textParts.join('')}</g>` + `<g id="Hot_Layer"></g>` + `<g id="Point_Layer"></g>`,
...
async function parseSymbolDefs(spcMap, onMissing) {
    ...
    for (const nodeName of Object.keys(spcMap)) {
        ...
        for (const symbolName of Object.keys(symbolMap)) {
            ...
            let s = SymbolParse.parse(symbol, symbolName, nodeName)
...
async function buildSvgDocument(gEl, layerEl, options) {
    ...
    if(spcMap['Sensitive']){
        spcMap['Sensitive']['diamond.mgtd.icn.g:diamond'] = true
        spcMap['Sensitive']['circle.mgtd.icn.g:circle'] = true
    } else {
        spcMap['Sensitive'] = {}
        spcMap['Sensitive']['diamond.mgtd.icn.g:diamond'] = true
        spcMap['Sensitive']['circle.mgtd.icn.g:circle'] = true
