import SymbolParse from "./SymbolParseKF.js";
export default class StationParseKf {
	symbolMap = {
		Disconnector: true,
	...
	getSpecialSymbol(list) {
		...
		list.each((index, dom) => {
			...
			if (devref && !map[nodeName]) {
				map[nodeName] = {};
			}
			if (devref) {
				let stopIndex = devref.indexOf(':');
				map[nodeName][devref.substring(1, stopIndex)] = true;
			}
		});
		return map;
	}
	...
	parseSymbol(spcMap) {
		...
		for(let key in spcMap) {
			...
			for(let symbolName in symbolMap) {
				...
				let url = './element/' + path + '/' + symbolName;
				$.ajax({
					type: 'post',
					url: url,
					dataType: 'xml',
