...
export default {
	...
	getTransform(transform) {
		let match1 = transform.match(this.regScale);
		let match2 = transform.match(this.regRotate);
		let param = {};
		if (match1) {
			let str = match1[0];
			let i1 = str.indexOf('(');
    ...
    getContainer(con, id) {
		...
		if (!w) {
			let _con = $(con).children()[0];
			w = _con.getAttribute('w');
			h = _con.getAttribute('h');
		}
	    this.symbolProps[id] = {
		    width: parseFloat(w),
		    height: parseFloat(h)
	...
	parseDev(dom) {
		...
		let lastFlagIndex = symbolId.lastIndexOf(':');
		symbolId = symbolId.substring(0, lastFlagIndex);
		let width, height;
		try {
    ...
    parseEle(dom) {
		...
		switch (nodeName) {
			case 'text':
				html = this.parseText(dom, false);
				break;
			case 'line':
				html = this.parseLine(dom, false);
				break;
			case 'ellipse':
				html = this.parseEllipse(dom, false);
				break;
			case 'polyline':
				html = this.parsePolyline(dom, false);
				break;
			case 'rect':
				html = this.parseRect(dom, false);
				break;
			case 'circle':
				html = this.parseCircle(dom, false);
				break;
			case 'polygon':
				html = this.parsePolygon(dom, false);
				break;
			case 'ellipsear':
				html = this.parseEllipsear(dom, false);
				break;
		}
		return html;
    },
    ...
    parse(symbol, id) {
		...
		let conStr = this.getContainer(symbol, id);
		sb.push(conStr);
		let list = $(symbol).find('Layer:first').children();
		list.each((index, dom) => {
